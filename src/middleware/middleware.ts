...
async function verifyToken(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const blacklistedToken = await redisClient.get('BL_' + decoded.sub?.toString() || '');
...
async function verifyRefreshToken(req: Request, res: Response, next: NextFunction) {
    ...
    if (!token) {
        return res.status(401).json({ status: false, message: "Invalid request. Token is missing." });
    }
    ...
    try {
        ...
        const data = await redisClient.get(decoded.sub?.toString() || '');
        ...
        if (JSON.parse(data).token !== token) {
            return res.status(401).json({ status: false, message: "Invalid request. Token does not match store." });
        }
